...
INTEGER(KIND=JPIM) :: NF,FBATCH,FBATCH_NEXT,NFIELDS
...
CALL GSTATS(158,0)
CALL TRGTOL_INIT(KF_FS,KF_GP,KF_SCALARS_G,IVSET,KPTRGP,&
 &PGP,PGPUV,PGP3A,PGP3B,PGP2,1,FBATCH)
CALL GSTATS(158,1)

FBATCH = FBATCH_DEF
DO NF = FBACTH_DEF+1,NFIELDS,FBATCH_DEF

   CALL TRGTOL_COMPLETE(PIN,NF-FBATCH_DEF,FBATCH,RECV_REQS,SEND_REQS,TOT_RECV_CNT,TOT_SEND_CNT)

   FBATCH_NEXT = MIN(FBATCH,NFIELDS-NF+1)
   
   CALL GSTATS(158,0)
   CALL TRGTOL_INIT(KF_FS,KF_GP,KF_SCALARS_G,IVSET,KPTRGP,&
        &PGP,PGPUV,PGP3A,PGP3B,PGP2,NF,FBATCH_NEXT)
   CALL GSTATS(158,1)
   CALL GSTATS(106,0)
...
!$OMP PARALLEL DO SCHEDULE(DYNAMIC,1) PRIVATE(JGL,IGL)
   DO JGL=IBEG,IEND,IINC
      IGL = JGL
      IF(KF_FS>0) THEN
         CALL FTDIR(PIN,ZGTF,KF_FS,IGL,NF-FBATCH,FBATCH)
      ENDIF
      
! Save Fourier data in FOUBUF_IN

      call gstats(811,0)
      CALL FOURIER_OUT(ZGTF,KF_FS,IGL,NF-FBATCH,FBATCH)
      call gstats(811,1)
...
DO JGL=IBEG,IEND,IINC
   IGL = JGL
   IF(KF_FS>0) THEN
      CALL FTDIR(PIN,ZGTF,KF_FS,IGL,NF-FBATCH_DEF,FBATCH)
   ENDIF
   
! Save Fourier data in FOUBUF_IN

   call gstats(811,0)
   CALL FOURIER_OUT(ZGTF,KF_FS,IGL,NF-FBATCH_DEF,FBATCH)
   call gstats(811,1)
