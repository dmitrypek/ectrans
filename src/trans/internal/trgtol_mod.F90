...
CALL TRGTOL_PROLOG(KF_FS,KF_GP,KVSET,&
 & ISENDCOUNT,IRECVCOUNT,ISEND,IRECV,IINDEX,INDOFF,IGPTRSEND)
...
SUBROUTINE TRGTOL_PROLOG(KF_FS,KF_GP,KVSET,&
 & KSENDCOUNT,KRECVCOUNT,KSEND,KRECV,KINDEX,KNDOFF,KGPTRSEND)
...
USE TPM_TRANS       ,ONLY : NGPBLKS,krecvtot_gtol,knrecv_gtol,trgtol_count,knsend_gtol,ksendtot_gtol
...
if(trgtol_count .eq. 0) then
   allocate(krecvtot_gtol(nproc))
   allocate(kSENDtot_gtol(nproc))
endif
...
!             count up expected number of fields
  ...
  ENDDO
  KSENDTOT_GTOL(JROC) = IGPTRRECV(ISETW)*IPOS

  IF( JROC /= MYPROC) THEN
    IF(KSENDTOT_GTOL(JROC) > 0) THEN
      KNSEND_GTOL = KNSEND_GTOL+1
      KSEND(KNSEND_GTOL)=JROC
    ENDIF
  ENDIF
  ...
  KRECVTOT_GTOL(JROC) = IPOS*KF_FS
  IF(KRECVTOT_GTOL(JROC) > 0 .AND. MYPROC /= JROC) THEN
    KNRECV_GTOL = KNRECV_GTOL + 1
    KRECV(KNRECV_GTOL)=JROC
  ENDIF
...
KRECVCOUNT=0
DO J=1,NPROC
  KSENDCOUNT=MAX(KSENDCOUNT,KSENDTOT_GTOL(J))
  KRECVCOUNT=MAX(KRECVCOUNT,KRECVTOT_GTOL(J))
ENDDO
...
!INTEGER(KIND=JPIM), INTENT(IN) :: KNSEND_GTOL
!INTEGER(KIND=JPIM), INTENT(IN) :: KNRECV_GTOL
!INTEGER(KIND=JPIM), INTENT(IN) :: KSENDTOT_GTOL (NPROC)
!INTEGER(KIND=JPIM), INTENT(IN) :: KRECVTOT_GTOL (NPROC)
...
!REAL(KIND=JPRB), ALLOCATABLE :: ZCOMBUFS_GTOL(:,:)
!REAL(KIND=JPRB), ALLOCATABLE :: ZCOMBUFR_GTOL(:,:)

!ALLOCATE(ZCOMBUFS_GTOL(-1:KSENDCOUNT,KNSEND_GTOL))
...
!IF (KNSEND_GTOL > 0 .AND. KSENDCOUNT >=-1) ZCOMBUFS_GTOL(-1,1)=HUGE(1._JPRB)

!ALLOCATE(ZCOMBUFR_GTOL(-1:KRECVCOUNT,KNRECV_GTOL))

CALL TRGTOL_COMM(PGLAT,KF_FS,KF_GP,KF_SCALARS_G,KVSET,&
 & KSENDCOUNT,KRECVCOUNT,KSEND,KRECV,KINDEX,KNDOFF,KGPTRSEND,&
 & KPTRGP,PGP,PGPUV,PGP3A,PGP3B,PGP2)

!DEALLOCATE(ZCOMBUFR_GTOL)
!DEALLOCATE(ZCOMBUFS_GTOL)
...
INTEGER(KIND=JPIM), INTENT(IN) :: KRECVCOUNT
!INTEGER(KIND=JPIM), INTENT(IN) :: KNSEND_GTOL
!NTEGER(KIND=JPIM), INTENT(IN) :: KNRECV_GTOL
!INTEGER(KIND=JPIM), INTENT(IN) :: KSENDTOT_GTOL (NPROC)
!NTEGER(KIND=JPIM), INTENT(IN) :: KRECVTOT_GTOL (NPROC)
INTEGER(KIND=JPIM), INTENT(IN) :: KSEND    (NPROC)
INTEGER(KIND=JPIM), INTENT(IN) :: KRECV    (NPROC)
INTEGER(KIND=JPIM), INTENT(IN) :: KINDEX(D%NLENGTF)
INTEGER(KIND=JPIM), INTENT(IN) :: KNDOFF(NPROC)
...
!REAL(KIND=JPRB) :: ZCOMBUFS_GTOL(-1:KSENDCOUNT,KNSEND_GTOL)
!REAL(KIND=JPRB) :: ZCOMBUFR_GTOL(-1:KRECVCOUNT,KNRECV_GTOL)
...
USE TPM_TRANS       ,ONLY : LDIVGP, LGPNORM, LSCDERS, LUVDER, LVORGP, NGPBLKS, &
     &                      PCOMBUFR_GTOL,trgtol_count,ireq_recv_gtol,knrecv_gtol,krecvtot_gtol, &
     &                      knsend_gtol,ksendtot_gtol,ireq_send_gtol,PCOMBUFS_GTOL
...
INTEGER(KIND=JPIM), INTENT(IN) :: KNDOFF(NPROC)
INTEGER(KIND=JPIM), INTENT(IN) :: KGPTRSEND(2,NGPBLKS,NPRTRNS)
!REAL(KIND=JPRB), INTENT(INOUT) :: PCOMBUFS_GTOL(-1:KSENDCOUNT,KNSEND_GTOL)
!REAL(KIND=JPRB), INTENT(INOUT) :: PCOMBUFR_GTOL(-1:KRECVCOUNT,KNRECV_GTOL)
INTEGER(KIND=JPIM) ,OPTIONAL, INTENT(IN) :: KPTRGP(:)
REAL(KIND=JPRB),OPTIONAL,INTENT(IN)     :: PGP(:,:,:)
REAL(KIND=JPRB),OPTIONAL,INTENT(IN)     :: PGPUV(:,:,:,:)
REAL(KIND=JPRB),OPTIONAL,INTENT(IN)     :: PGP3A(:,:,:,:)
...
INTEGER(KIND=JPIM) :: IPOSPLUS(KNSEND_GTOL)
INTEGER(KIND=JPIM) :: ISETW(KNSEND_GTOL)
INTEGER(KIND=JPIM) :: IJPOS(NGPBLKS,KNSEND_GTOL)
INTEGER(KIND=JPIM) :: IFLDA(KF_GP,KNSEND_GTOL)
!INTEGER(KIND=JPIM) :: IREQ_SEND_GTOL(NPROC)
!INTEGER(KIND=JPIM) :: IRECV_RECV_GTOL(NPROC)
...
IF(TRGTOL_COUNT .EQ. 0) THEN
   ALLOCATE(PCOMBUFR_GTOL(-1:KRECVCOUNT,KNRECV_GTOL))
   ALLOCATE(IREQ_RECV_GTOL(KNRECV_GTOL))
   ALLOCATE(PCOMBUFS_GTOL(-1:KSENDCOUNT,KNSEND_GTOL))
   ALLOCATE(IREQ_SEND_GTOL(KNSEND_GTOL))
ENDIF
...
!   if(trgtol_count .eq. 0) THEN
   !  Receive loop.........................................................
  CALL GSTATS(1504,0)
      DO INR=1,KNRECV_GTOL
         IRECV=KRECV(INR)
         CALL MPL_RECV(PCOMBUFR_GTOL(-1:KRECVTOT_GTOL(IRECV),INR), &
              & KSOURCE=NPRCIDS(IRECV), &
              & KMP_TYPE=JP_NON_BLOCKING_STANDARD,KREQUEST=IREQ_RECV_GTOL(INR), &
              & KTAG=ITAG,CDSTRING='TRGTOL_COMM: NON-BLOCKING IRECV' )
      ENDDO
  CALL GSTATS(1504,1)
!   else
!      CALL GSTATS(1505,0)
!      IF(KNSEND_GTOL > 0) THEN
!         CALL MPL_WAIT(KREQUEST=IREQ_SEND_GTOL(1:KNSEND_GTOL),CDSTRING='TRGTOL_COMM: WAIT FOR ISENDS')
!      ENDIF
!      CALL GSTATS(1505,1)
!   endif      
ENDIF
...
IF(KSENDTOT_GTOL(MYPROC) > 0 )THEN
  IFLDS = 0
  DO JFLD=1,KF_GP
...
DO INS=1,KNSEND_GTOL
  ...
  PCOMBUFS_GTOL(-1,INS) = 1
  PCOMBUFS_GTOL(0,INS) = IFLD
...
DO INS=1,KNSEND_GTOL
  ...
  ISEND_FLD_START=PCOMBUFS_GTOL(-1,INS)
  ISEND_FLD_END = PCOMBUFS_GTOL(0,INS)
      ...
      IF(IFIRST > 0) THEN
        ...
        IF(LLINDER) THEN
          DO JK=IFIRST,ILAST
            ...
            PCOMBUFS_GTOL(JI,INS) = PGP(JK,KPTRGP(IFLDT),JBLK)
          ENDDO
        ELSE
          IF(LLPGPONLY) THEN
            DO JK=IFIRST,ILAST
              JI=(JJ-ISEND_FLD_START)*IPOS+IJPOS(JBLK,INS)+JK-IFIRST+1
              PCOMBUFS_GTOL(JI,INS) = PGP(JK,IFLDT,JBLK)
            ENDDO
          ELSEIF(LLUV(IFLDT)) THEN
            DO JK=IFIRST,ILAST
              JI=(JJ-ISEND_FLD_START)*IPOS+IJPOS(JBLK,INS)+JK-IFIRST+1
              PCOMBUFS_GTOL(JI,INS) = PGPUV(JK,IUVLEVS(IFLDT),IUVPARS(IFLDT),JBLK)
            ENDDO
          ELSEIF(LLGP2(IFLDT)) THEN
            DO JK=IFIRST,ILAST
              JI=(JJ-ISEND_FLD_START)*IPOS+IJPOS(JBLK,INS)+JK-IFIRST+1
              PCOMBUFS_GTOL(JI,INS) = PGP2(JK,IGP2PARS(IFLDT),JBLK)
            ENDDO
          ELSEIF(LLGP3A(IFLDT)) THEN
            DO JK=IFIRST,ILAST
              JI=(JJ-ISEND_FLD_START)*IPOS+IJPOS(JBLK,INS)+JK-IFIRST+1
              PCOMBUFS_GTOL(JI,INS) = PGP3A(JK,IGP3ALEVS(IFLDT),IGP3APARS(IFLDT),JBLK)
            ENDDO
          ELSEIF(LLGP3B(IFLDT)) THEN
            DO JK=IFIRST,ILAST
              JI=(JJ-ISEND_FLD_START)*IPOS+IJPOS(JBLK,INS)+JK-IFIRST+1
              PCOMBUFS_GTOL(JI,INS) = PGP3B(JK,IGP3BLEVS(IFLDT),IGP3BPARS(IFLDT),JBLK)
            ENDDO
          ENDIF
        ENDIF
      ENDIF
...
!$OMP END PARALLEL DO
  ...
  IF (NTRANS_SYNC_LEVEL <= 1) THEN
     CALL MPL_SEND(PCOMBUFS_GTOL(-1:KSENDTOT_GTOL(ISEND),INS),KDEST=NPRCIDS(ISEND), &
          & KMP_TYPE=JP_NON_BLOCKING_STANDARD,KREQUEST=IREQ_SEND_GTOL(INS), &
...
!  Unpack loop.........................................................
  ...
  CALL GSTATS(1503,0)
DO JNR=1,KNRECV_GTOL

  IF (NTRANS_SYNC_LEVEL <= 0) THEN
     CALL MPL_WAITANY(KREQUEST=IREQ_RECV_GTOL(1:KNRECV_GTOL),KINDEX=INR,&
  ...
  IRECV=KRECV(INR)
  ILEN = KRECVTOT_GTOL(IRECV)/KF_FS
  IRECV_FLD_START = PCOMBUFR_GTOL(-1,INR)
  IRECV_FLD_END   = PCOMBUFR_GTOL(0,INR)
  CALL GSTATS(1500,0)
!$OMP PARALLEL DO SCHEDULE(STATIC) PRIVATE(JL,II,JFLD)
  DO JL=1,ILEN
    II = KINDEX(KNDOFF(IRECV)+JL)
    DO JFLD=IRECV_FLD_START,IRECV_FLD_END
      PGLAT(JFLD,II) = PCOMBUFR_GTOL(JL+(JFLD-IRECV_FLD_START)*ILEN,INR)
    ENDDO
  ENDDO
!$OMP END PARALLEL DO
  CALL GSTATS(1500,1)
