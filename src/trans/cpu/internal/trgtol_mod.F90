...
SUBROUTINE TRGTOL(PGLAT,KF_FS,KF_GP,KF_SCALARS_G,KVSET,KPTRGP,ZCOMBUFS,ZCOMBUFR,SEND_ID,RECV_ID,&
...
IF (NSTACK_MEMORY_TR==0) THEN
  CALL TRGTOL_COMM_HEAP(PGLAT,KF_FS,KF_GP,KF_SCALARS_G,KVSET, &
   & ISENDCOUNT,IRECVCOUNT,INSEND,INRECV,ISENDTOT,IRECVTOT,ISEND,IRECV,IINDEX,INDOFF,IGPTRSEND, &
   & KPTRGP,ZCOMBUFS,ZCOMBUFR,SEND_ID,RECV_ID,PGP,PGPUV,PGP3A,PGP3B,PGP2,LUSE_WAITANY)
ELSE
  CALL TRGTOL_COMM_STACK(PGLAT,KF_FS,KF_GP,KF_SCALARS_G,KVSET, &
   & ISENDCOUNT,IRECVCOUNT,INSEND,INRECV,ISENDTOT,IRECVTOT,ISEND,IRECV,IINDEX,INDOFF,IGPTRSEND, &
   & KPTRGP,SEND_ID,RECV_ID,PGP,PGPUV,PGP3A,PGP3B,PGP2,LUSE_WAITANY)
ENDIF
...
CALL TRGTOL_COMM(PGLAT,KF_FS,KF_GP,KF_SCALARS_G,KVSET,&
 & KSENDCOUNT,KRECVCOUNT,KNSEND,KNRECV,KSENDTOT,KRECVTOT,KSEND,KRECV,KINDEX,KNDOFF,KGPTRSEND,&
 & ZCOMBUFS,ZCOMBUFR,SEND_ID,RECV_ID, &
 & KPTRGP,PGP,PGPUV,PGP3A,PGP3B,PGP2,LUSE_WAITANY)
...
CALL TRGTOL_COMM(PGLAT,KF_FS,KF_GP,KF_SCALARS_G,KVSET,&
 & KSENDCOUNT,KRECVCOUNT,KNSEND,KNRECV,KSENDTOT,KRECVTOT,KSEND,KRECV,KINDEX,KNDOFF,KGPTRSEND,&
 & ZCOMBUFS_STACK,ZCOMBUFR_STACK,SEND_ID,RECV_ID, &
 & KPTRGP,PGP,PGPUV,PGP3A,PGP3B,PGP2,LUSE_WAITANY)
...
SUBROUTINE TRGTOL_COMM(PGLAT,KF_FS,KF_GP,KF_SCALARS_G,KVSET,&
 & KSENDCOUNT,KRECVCOUNT,KNSEND,KNRECV,KSENDTOT,KRECVTOT,KSEND,KRECV,KINDEX,KNDOFF,KGPTRSEND,&
 & PCOMBUFS,PCOMBUFR, &
 & SEND_ID,RECV_ID,KPTRGP,PGP,PGPUV,PGP3A,PGP3B,PGP2,LUSE_WAITANY)
  ...
  DO JJ=ISEND_FLD_START,ISEND_FLD_END
    ...
    DO JBLK=1,NGPBLKS
      ...
      IF(IFIRST > 0) THEN
        ...
        IF(LLINDER) THEN
          DO JK=IFIRST,ILAST
            PCOMBUFS(JI+JK,INS) = PGP(JK,KPTRGP(IFLDT),JBLK)
          ENDDO
        ELSE
          IF(LLPGPONLY) THEN
            DO JK=IFIRST,ILAST
              PCOMBUFS(JI+JK,INS) = PGP(JK,IFLDT,JBLK)
            ENDDO
          ELSEIF(LLUV(IFLDT)) THEN
            DO JK=IFIRST,ILAST
              PCOMBUFS(JI+JK,INS) = PGPUV(JK,IUVLEVS(IFLDT),IUVPARS(IFLDT),JBLK)
            ENDDO
          ELSEIF(LLGP2(IFLDT)) THEN
            DO JK=IFIRST,ILAST
              PCOMBUFS(JI+JK,INS) = PGP2(JK,IGP2PARS(IFLDT),JBLK)
            ENDDO
          ELSEIF(LLGP3A(IFLDT)) THEN
            DO JK=IFIRST,ILAST
              PCOMBUFS(JI+JK,INS) = PGP3A(JK,IGP3ALEVS(IFLDT),IGP3APARS(IFLDT),JBLK)
            ENDDO
          ELSEIF(LLGP3B(IFLDT)) THEN
            DO JK=IFIRST,ILAST
              PCOMBUFS(JI+JK,INS) = PGP3B(JK,IGP3BLEVS(IFLDT),IGP3BPARS(IFLDT),JBLK)
            ENDDO
          ENDIF
        ENDIF
      ENDIF
