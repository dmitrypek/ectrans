MODULE OVERLAP_TYPES_MOD
  ...
  IMPLICIT NONE
    ...
    INTEGER(KIND=JPIM), ALLOCATABLE :: ILENS(:),ILENR(:),IOFFS(:),IOFFR(:), IREQ_SEND(:), IREQ_RECV(:)
...
CONTAINS
  ...
  FUNCTION BATCH_CONSTRUCTOR(KBLK, KF_GP, KF_SCALARS_G, KF_UV_G, KVSETUV, KVSETSC, IOFFSEND, &
       ...
       &                        NPTRGP,NPTRSPUV,NPTRSPSC) RESULT(THIS)
  ...
  SUBROUTINE INIT_SENDS(THIS, PCOMBUFS,IREQ_SEND,SEND_ID)
...
!    IF(THIS%STAGE .EQ. 1) THEN
       ...
       DO INS=1,NREQ
          ...
          CALL MPI_SEND_INIT(PCOMBUFS(THIS%MYOFFSEND:THIS%MYOFFSEND+THIS%NSENDTOT(ISEND)-1,INS), &
                    ...
                    &                 dest, MTAGGL + this%nblk, MPI_COMM_WORLD, IREQ_SEND(INS),IERR)
       ENDDO
...
!    ENDIF
  ...
  END SUBROUTINE INIT_SENDS
    ...
    SUBROUTINE INIT_RECVS(THIS, PCOMBUFR,IREQ_RECV,RECV_ID)
...
!    IF(THIS%STAGE .EQ. 1) THEN
       ...
       DO INR=1,NREQ
          ...
          CALL MPI_RECV_INIT(PCOMBUFR(THIS%MYOFFRECV:THIS%MYOFFRECV+THIS%NRECVTOT(IRECV)-1,INR), &
                    ...
                    &                 src, itag, MPI_COMM_WORLD, IREQ_RECV(INR),IERR)
       ENDDO
       CALL PT_REQSET_REGISTER(NREQ,IREQ_RECV,FLG,RECV_ID,STATUS)
       IF(STATUS .EQ. MPI_ERR_ARG) THEN
...
!    ENDIF
  ...
  END SUBROUTINE INIT_RECVS
    ...
    SUBROUTINE START_COMM(THIS, PGP, PCOMBUFS, PIN)
...
!    REAL(KIND=JPRB),           INTENT(INOUT) :: PCOMBUFR(:,:)
    ...
    SELECT CASE(THIS%STAGE)
      CASE (1)
        CALL TRGTOL_COMM_SEND(PIN(:,THIS%IOFFGTF:THIS%IOFFGTF+THIS%NF_FS-1), PCOMBUFS,  &
          &                   THIS%MYOFFSEND, THIS%MYOFFRECV, this%ireq_recv,THIS%NF_FS, THIS%NF_GP, &
