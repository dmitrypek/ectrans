MODULE OVERLAP_TYPES_MOD
  ...
  PRIVATE
    ...
    INTEGER(KIND=JPIM), ALLOCATABLE :: ILENS(:),ILENR(:),IOFFS(:),IOFFR(:), IREQ_SEND(:)
    ...
    SELECT CASE(THIS%STAGE)
      CASE (1)
        CALL TRGTOL_COMM_SEND(PIN(:,THIS%IOFFGTF:THIS%IOFFGTF+THIS%NF_FS-1), PCOMBUFS,  &
          &                   THIS%MYOFFSEND, THIS%NF_FS, THIS%NF_GP, &
...
!    INTEGER(KIND=JPIM), INTENT(INOUT) :: IREQ_RECV(:)
    ...
    CALL PT_REQSET_TEST(THIS%RECV_ID(THIS%STAGE),FLAG,STATUS)
    IF(STATUS .EQ. MPI_ERR_ARG) THEN
